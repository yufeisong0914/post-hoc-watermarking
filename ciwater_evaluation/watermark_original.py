...
project_root = "/home/haojifei/develop_things/nlp_projects/wm02-1/"
...
class watermark_model:
    def __init__(self, language, mode, tau_word, lamda):
        ...
        if language == 'Chinese':
            self.relatedness_tokenizer = AutoTokenizer.from_pretrained(
                source_root + "IDEA-CCNL/Erlangshen-Roberta-330M-Similarity")
            self.relatedness_model = AutoModelForSequenceClassification.from_pretrained(
                source_root + "IDEA-CCNL/Erlangshen-Roberta-330M-Similarity"
            ).to(self.device)

            self.tokenizer = WoBertTokenizer.from_pretrained(source_root + "junnyu/wobert_chinese_plus_base")
            self.model = WoBertForMaskedLM.from_pretrained(
                source_root + "junnyu/wobert_chinese_plus_base",
            ...
            self.relatedness_model = RobertaForSequenceClassification.from_pretrained(
                source_root + 'roberta-large-mnli'
            ).to(self.device)
            self.relatedness_tokenizer = RobertaTokenizer.from_pretrained(
                source_root + 'roberta-large-mnli'
            )

            self.tokenizer = BertTokenizer.from_pretrained(source_root + 'bert-base-cased')
            self.model = BertForMaskedLM.from_pretrained(
                source_root + 'bert-base-cased',
                output_hidden_states=True
            ).to(self.device)
    ...
    def filter_special_candidate(self, top_n_tokens, tokens, masked_token_index, input_text):
        if self.language == 'English':
            ...
            processed_tokens = [base_word] + [tok for tok in filtered_tokens if
                                              self.nlp(tok)[0].lemma_ != base_word_lemma]
    ...
    def context_word_sim(self, init_candidates, tokens, masked_token_index, input_text):
        ...
        batch_input_ids = [[self.tokenizer.convert_tokens_to_ids(
            ['[CLS]'] + tokens[1:masked_token_index] + [token] + tokens[masked_token_index + 1:-1] + ['[SEP]'])] for
            token in init_candidates]
    ...
    def watermark_embed(self, text):
        ...
        for masked_token_index in range(start_index + 1, end_index - 1):
            ...
            for i, tok in enumerate(enhanced_candidates):
                ...
                if binary_encoding != 1 or (is_similar(tok[0], tokens[masked_token_index])) or (
                        tokens[masked_token_index - 1] in tok or tokens[masked_token_index + 1] in tok):
